...
Hence env.agent[n].goal_pixel can by taken as the output of Imitation Learning since the, neural network is expected to operate on (I/O means) gmapping map's pixel coordinates.
...
Control functions:
Note-4: Most of the control functions are demonstrated in the above code itself. Some of the special functions are mentioned below:
env.agent[n].switch_to_free_nav() - If an agent is initialized with an exploration package, but neural network need to take over this needs to be executed before set_goal_pixel()
...
env.step() - executed exactly 1 step of the environment (executes steps function of all agents parallely once, for one step movement or no movement)
